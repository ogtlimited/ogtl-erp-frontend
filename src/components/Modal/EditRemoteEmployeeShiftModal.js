...
export const EditRemoteEmployeeShiftModal = ({
  ...
  useEffect(() => {
    ...
    const formattedEmployeeShifts = employeeRemoteShifts?.map((shift) => ({
      day: shift.day,
      off: shift.off,
    }));
  ...
  }, [employeeRemoteShifts, officeID, userID]);
  ...
  const handleEditEmployeeShift = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.patch(
        `/api/v1/remote_attendance_shifts/${employeeID}.json`,
        {
          headers: {
      ...
      setLoading(false);
      goToTop();
      showAlert(true, `Remote shift updated successfully!`, "alert alert-success");
    } catch (error) {
      goToTop();
      ...
      <div className="card profile-box flex-fill">
        <div className="card-body">
          <div className="col" style={{ marginBottom: "30px" }}>
            <h4>Edit Remote Employee Shift</h4>
          </div>
