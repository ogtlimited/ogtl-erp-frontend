...
function OperationsResignationFeedbackModal({
  setmodalType,
  resignationContent,
  ...
  const handleResignationFeedback = async (e) => {
    ...
    try {
      ...
      const response = await axiosInstance.put(
        `/api/v1/operation_team_approve_resignations/${id}.json`,
        {
          payload: {
            feedback: data,
          },
      ...
      showAlert(
        true,
        `${resignationContent?.full_name} resignation application has been moved to the next stage`,
        "alert alert-info"
      );
...
export default OperationsResignationFeedbackModal;
