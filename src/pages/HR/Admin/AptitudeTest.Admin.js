...
const AptitudeTest = () => {
    ...
    useEffect(() => {
        combineRequest().then(res => {
            console.log(res)
            const {jobApplicants,employees} = res.data.createEmployeeFormSelection
    ...
    useEffect(() => {
        ...
        if(submitted === true){
            axiosInstance.post('/api/test', formValue).then(res =>{
                setSubmitted(false);
                fetchAllTests()
    ...
    const deleteTest = (row) =>{
        axiosInstance.delete(`/api/test/${row._id}`).then(res =>{
            console.log(res)
    ...
    const updateTest = (row) =>{
        axiosInstance.patch(`/api/test/${row._id}`,row).then(res =>{
            console.log(res)
            setData(prevData => [...data, res.data.data])
            fetchAllTests()
            showAlert(true,res.data.message,'alert alert-success')
    ...
    const columns = [
        ...
        },
        {
            ...
            sort: true,
            formatter: (value, row) => (
                <h2>{row?.hr_user?.first_name} {row?.hr_user?.last_name}</h2>
            )
            ...
            formatter: (value, row) => (
                <div className="dropdown dropdown-action text-right"><a href="#" className="action-icon dropdown-toggle" data-toggle="dropdown"
                    ...
                    <div className="dropdown-menu dropdown-menu-right">
                        ...
                        <Link className="dropdown-item" onClick={() => deleteTest(row)}><i className="fa fa-trash m-r-5"></i> Delete</Link>
                    </div>
                </div>
            )    ,
        },
    ...
    return (
        <>
            ...
            <FormModal setformValue={setFormValue} template={applicationTestFormJson} setsubmitted={setSubmitted}  />
        </>
    );
};
