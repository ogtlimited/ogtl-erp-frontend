...
const ShiftAssignment = () => {
    ...
    const fetchShiftAssignments = () =>{
        axiosInstance.get('/api/shiftAssignment').then(res=>{
    ...
    useEffect(() => {
        ...
        if(submitted === true){
            axiosInstance.post('/api/shiftAssignment', formValue).then(res =>{
                setSubmitted(false);
                fetchShiftAssignments()
    ...
    const deleteShiftAssignment = (row) =>{
        axiosInstance.delete(`/api/shiftAssignment/${row._id}`).then(res =>{
            console.log(res)
    ...
    const updateShiftAssignment = (row) =>{
        axiosInstance.patch(`/api/shiftAssignment/${row._id}`,row).then(res =>{
            console.log(res)
            setData(prevData => [...data, res.data.data])
            fetchShiftAssignments()
            showAlert(true,res.data.message,'alert alert-success')
            ...
            formatter: (value, row) => (
                <div className="dropdown dropdown-action text-right"><a href="#" className="action-icon dropdown-toggle" data-toggle="dropdown"
                    ...
                    <div className="dropdown-menu dropdown-menu-right">
                        ...
                        <Link className="dropdown-item" onClick={() => deleteShiftAssignment(row)} ><i className="fa fa-trash m-r-5"></i> Delete</Link>
                    </div>
                </div>
            )    ,
        },
