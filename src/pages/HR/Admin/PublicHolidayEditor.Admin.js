...
const PublicHolidayEditor = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const publicHoliday = location?.state;

  console.log("publicHoliday param:", publicHoliday);
  ...
  const [publicHolidayData, setPublicHolidayData] = useState({
    ...
    start_date: moment(publicHoliday?.start_date)
      .utc()
      .format("YYYY-MM-DDTHH:mm"),
    end_date: moment(publicHoliday?.end_date).utc().format("YYYY-MM-DDTHH:mm"),
  });
  ...
  const fetchPublicHolidayDepartment = useCallback(async () => {
    try {
      const response = await axiosInstance.get(
        `/api/v1/department_holidays/${publicHoliday?.id}.json`,
        {
          headers: {
      ...
      console.log("fetchPublicHolidayDepartment", response);
    } catch (error) {
...
export default PublicHolidayEditor;
